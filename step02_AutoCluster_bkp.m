...
%nSamples=32; %32 can be adjusted to cluster using longer or shorter spikes, spikes will be truncated before writing to *.NSE files
%peakSample=8; %8; 7 samples before and 24 after the peak will be written to *.NSE file
...
%                     if A3<-minHeight/2 && P4<-minHeight/2 && P5>minHeight*1.5 % => initial hump selected => check valley and next peak
%                         if T5-T4+1>10
%                             % Haps peak=P5 (Type 1)
%                             type=1;
%                             spikeTime=T4;
...
%                             amplitude=abs(A4);
%                             width=T5-T4+1;
%                         else % shift to second large peak
%                             type=3;
%                             spikeTime=T5;
%                             trigger_value=P5;
%                             valley_value=P6;
...
%                             width=T6-T5+1;
%                         end
%                     %elseif P2<-minHeight/2 && P3>minHeight*1.5 % peak selected => check if pre-valley is low enough
%                     elseif P2<-minHeight/2 && P3>minHeight*1.5 && T3-T2+1>10 % peak selected => check if pre-valley is low enough                    
%                         % If P2 < -minHeight and P3 > minHeight =>
%                         % HAPS peak=P3 (type 2)
%                         type=2;
%                         spikeTime=T2;
%                         trigger_value=P2;
...
%                     end
                ...
                if type>0
                    ...
                    %waveForms(count).waveform=Y(spikeTime-23:spikeTime+48)';
                    count=count+1;
                end
            ...
            switch 1
                ...
                case 2 % relaxing threshold: not used for now as we don't find significant advantages over regular method (1)
                    %%
                    % start with high threshold, copy selected
                    % spikes to result-vector and remove
                    % retriggerSamples in both direction from the
                    % selected peaks => results in a top-down
                    % selection of high-amplitude spikes
                    amplitude_vector=zeros(size(Y));
                    for iAmpl=1:size(peak_data_minHeigth,1)
                        amplitude_vector(peak_data_minHeigth(iAmpl,4))=peak_data_minHeigth(iAmpl,7);
                    end
                    
                    %%%
                    result_vector=zeros(size(amplitude_vector));
                    TH_vector=[400:-50:50 minHeight/2];
                    for iThreshold=1:length(TH_vector)
                        TH=TH_vector(iThreshold);
                        sel=amplitude_vector>TH;
                        result_vector(sel)=amplitude_vector(sel);
                        %plot(result_vector)
                        %drawnow
                        
                        %% Remove spikes around selected ones
                        removal_vector=conv(double(sel),[0 ones(1,retriggerSamples*2) 0],'same')>=1;
                        amplitude_vector(removal_vector>=1)=0;
                    end
                    sel=ismember(peak_data_minHeigth(:,4),find(result_vector>0));
    ...
    if clusterIt==1
        if runClusterAnalysis==1
            ...
            toc
            save(matFilename,'ClusterAllocation','nSpikes','nSpikesPerCluster','nClusters','PCA_string','-append')
        else
            disp('Loading existing cluster allocations...')
            load(matFilename,'ClusterAllocation','nSpikes','nSpikesPerCluster','nClusters','PCA_string')
        end
        clusterType='SUA';
    else
        ...
        save(matFilename,'ClusterAllocation','nSpikes','nSpikesPerCluster','nClusters','-append')
        clusterType='MUA';
    end
